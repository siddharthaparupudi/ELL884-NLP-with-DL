...
def naiveSoftmaxLossAndGradient(
    ...
    word_dim = centerWordVec.shape[0]
    vocab_dim = y_hat.shape[0]
    
    y = np.zeros(vocab_dim)
    y[outsideWordIdx] = 1
    
    gradCenterVec = np.zeros(word_dim)
    gradCenterVec = np.dot(outsideVectors.T,y_hat-y)    

    gradOutsideVecs = np.zeros((vocab_dim,word_dim))
    for i in range(vocab_dim):
        if i == outsideWordIdx:
            gradOutsideVecs[i] = centerWordVec*(y_hat[i]-1)  
        else:
            gradOutsideVecs[i] = centerWordVec*(y_hat[i])  
...
def negSamplingLossAndGradient(
    ...
    word_dim = centerWordVec.shape[0]
    vocab_dim = outsideVectors.shape[0]
    
    negWordsMatrix = np.zeros((K+1,word_dim))
    negWordsMatrix[0] = outsideVectors[outsideWordIdx]
    for i in range(K):
        negWordsMatrix[i+1] = -outsideVectors[indices[i+1]]
    
    sig = sigmoid(np.dot(negWordsMatrix,centerWordVec)) 
    ...
    gradCenterVec = np.zeros(word_dim)
    gradCenterVec += outsideVectors[outsideWordIdx]*(sig[0]-1)
    for i in range(K):
        gradCenterVec -= outsideVectors[indices[i+1]]*(sig[i+1]-1)
    
    
    gradOutsideVecs = np.zeros((vocab_dim,word_dim))
    gradOutsideVecs[outsideWordIdx] += centerWordVec*(sig[0]-1)
    for i in range(K):
        gradOutsideVecs[indices[i+1]] += -centerWordVec*(sig[i+1]-1)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    currentCenterVec = centerWordVectors[currentCenterInd]
    
    for outsideWord in outsideWords:
        outsideWordIdx = word2Ind[outsideWord]
        stepLoss, gradCenter, gradOutside = word2vecLossAndGradient(currentCenterVec,outsideWordIdx,outsideVectors,dataset)

        loss += stepLoss
        gradCenterVecs[currentCenterInd] += gradCenter
